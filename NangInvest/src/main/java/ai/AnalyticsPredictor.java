...
public class AnalyticsPredictor {
    ...
    public Map<String, Double> predictCoursePopularity() {
        ...
        for (Course course : allCourses) {
            ...
            long recentOrders = orderDAO.findByProductId(course.getCourseId()).stream().filter(order -> isWithinDays(order.getOrderId(), 30)).count();
    ...
    public Map<String, Object> getPlatformAnalytics() {
        ...
        //couse metrics
        analytics.put("totalCourses", courseDAO.count());
        ...
        analytics.put("totalBlogs", blogDAO.count());
    ...
    private String extractTopicFromCourse(Course course){
        ...
        if (name.contains("stock") || name.contains("trading")) return "Trading";
        if (name.contains("investment") || name.contains("invest")) return "Investment";
        if (name.contains("retirement") || name.contains("plan")) return "Retirement Plan";
